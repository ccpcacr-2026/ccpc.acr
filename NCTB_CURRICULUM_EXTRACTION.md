...
| Secondary (Class 6–9) | Teacher's Guide (newer 2026 curriculum, 13 subjects/class) | **Found and linked** — see `NCTB_TEXTBOOK_LINKS.md` | Located via targeted search for each class individually (not the combined Pre-primary/Primary/Secondary list page, which only actually lists Pre-Primary and Primary — Secondary pages exist standalone, some on `nctb.gov.bd`, some on `nctb.portal.gov.bd`). **Important**: this newer curriculum's 13-subject structure (Bangla, English, Math, Science, History & Social Science, Digital Technology, Health Protection, Life & Livelihood, Arts & Culture, 4 religions) does not line up subject-for-subject with the older-curriculum Textbook links already in `NCTB_BOOKS`/`NCTB_TEXTBOOK_LINKS.md` (~20 more granular subjects). None of Six-Nine has been read/transcribed yet — extraction not started. |
| Class 10 | — | **Not yet found** | Nine's Teacher's Guide is published as its own standalone grade (not combined with Ten) — a separate Class Ten page likely exists but hasn't been located yet; check nctb.gov.bd/nctb.portal.gov.bd with the same per-class targeted search that worked for Six-Nine. |
...
thread small regardless of how many chapters/subjects/classes remain.
  - [~] **English (English for Today) — IN PROGRESS.** Source: TG `D:\NCTB books\Class One\English Version\English - Teacher's Guide.pdf` (182 pages) + TB `...\English - Textbook.pdf` (98 pages), both plain English text (not Bijoy-Bangla — TG instructional prose is Bangla but activity content is English, textbook is pure English), much easier to read than Math. **Important subject-tag correction vs Math**: `lesson_curricula.subject` must be `'English for Today'`, not `'English'` — the Lesson Plan form's Subject dropdown and AI-draft book-URL resolution both key off `NCTB_BOOKS[className]` entries verbatim (`app.js` `_lpSubjectOptionsList`/`_lpGenerateWithAi`), and Class One's NCTB_BOOKS entry is literally `'English for Today'`.
    ...
    - **Unit 3 (Classroom Instructions) — COMPLETE, live as id 20, 7/7 sessions** (3 Lessons: Classroom Instructions 1 (2 sessions), Classroom Instructions 2 (2 sessions), Classroom Instructions 3 (3 sessions) — matches the pre-verified TOC map exactly; confirmed against actual TG page content through the "Concluding the session" on TG printed page 150, right before Unit 4's header begins). Both `elaborate_summary` (TG) and `textbook_context` (TB) populated for all 7. TG PDF pages read: 149-158 (printed pages 141-150). TB PDF pages read: 69-77 (printed pages 65-73). Note: source TG has a minor internal inconsistency, repeating "Classroom commands N" as a session-header variant of "Classroom Instructions N" partway through Lessons 2 and 3 — standardized on "Classroom Instructions N" (matching the Textbook's consistent heading) for all `topic` fields.
    - **Unit 4 (Questions and Answers) — COMPLETE, live as id 21, 8/8 sessions** (6 Lessons: Self-introduction (2 sessions), In the Village (2 sessions), In the City (1), At Village Home (1), In the Classroom (1), Review Lesson (1) — matches the pre-verified TOC map exactly; confirmed against actual TG content through "Concluding the session" on TG printed page 161, right before Unit 5's header begins). Lesson 6 "Review Lesson" (TB page 84) confirmed to be a cumulative game-based review (flashcard sorting game reusing village/city vocabulary from Lessons 2-5, a ball-toss chain game reusing Lesson 1's self-introduction question, and a draw-and-display task) rather than new content — transcribed faithfully to that structure. Both `elaborate_summary` (TG) and `textbook_context` (TB) populated for all 8. TG PDF pages read: 159-169 (printed pages 151-161). TB PDF pages read: 79-88 (printed pages 75-84).
