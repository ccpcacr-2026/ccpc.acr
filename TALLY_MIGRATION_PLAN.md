...
- **Done (2026-09-15): visual rework to match real TallyPrime.** The first UI pass emulated a dark DOS-era Tally theme, based on an assumption about what "TallyPrime Gold" looked like — the user then shared real TallyPrime EDU screenshots (Gateway of Tally, Company Creation) showing the actual modern software: dark-teal menu bar, light body, pale-blue Gateway card with a navy header and amber/gold selection highlight, plain sans-serif type. Rebuilt the whole `tp-*` palette in `_src/styles.css` to match, added a real top menu bar (K:Company/Y:Data/Z:Exchange/G:Go To/O:Import/E:Export/M:Share/P:Print/F1:Help), and changed the Gateway panel to match real Tally exactly — the Gateway-of-Tally menu is now the floor screen's own centered content, not a menu permanently pinned in the right panel on every screen (an earlier, since-superseded request); the right panel is now a small contextual sliver (F2:Date/F3:Company), same as real Tally.
- **Done (2026-09-15): Import/Export/Print wired up.** Import bulk-loads Ledgers or Vouchers from Excel (mirrors Payroll's existing importer exactly — column-mapping UI, per-row error reporting) via a new `import_rows` action in `app/api/accounts-admin/route.js`. It can **also** read a real TallyPrime XML export directly (Gateway of Tally → Export → Masters/Vouchers) — parsed client-side, converted to the same row shape the Excel path uses, then the identical preview-and-confirm step. **This XML parser is written against Tally's documented export schema, not yet checked against a real exported file — see Next Steps.** Export produces an Excel file of whatever table is on screen; Print opens a clean print window. Company/Data/Exchange/Go To/Share stay inert with an explanatory toast (not relevant to a single institution).

## Next steps

1. **User: click-test the new Tally-style UI** in a real browser (desktop and a phone) — try F4-F9, Esc, Ctrl+A, Alt+C, the Gateway menu's arrow keys/hotkeys, and confirm nothing feels broken. Report anything that doesn't behave as expected. Also sanity-check the new color palette against the real TallyPrime screenshots and flag anything that reads wrong.
2. **User: export a small Masters and/or Vouchers XML from real TallyPrime** (Gateway of Tally → Export) and try importing it via the new Import screen's XML path — the parser was written against Tally's documented export schema without a real file to verify against, so this is the first real test of it. Report anything that doesn't parse or maps to the wrong field.
3. Once the XML importer is confirmed correct against a real small sample, do the full historical Masters + Vouchers export/import, and manually reconcile a sample (e.g. one ledger's balance) against Tally's own report for the same ledger before trusting it.
4. Only after the above is verified correct: start treating this app as the system of record and wind down day-to-day Tally use.
